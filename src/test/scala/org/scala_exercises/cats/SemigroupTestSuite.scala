...
class SemigroupTestSuite extends AnyFunSuiteLike with Matchers {
  test("test CATS LIB section Semigroup 0") {
  ...
  test("test CATS LIB section Semigroup 1") {
    Semigroup[Int => Int].combine(_ + 1, _ * 10).apply(6) should be(67)
  }

  test("test CATS LIB section Semigroup 2") {
  ...
  test("test CATS LIB section Semigroup 3") {
