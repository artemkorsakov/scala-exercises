...
class CaseClassesTestSuite extends AnyFunSuiteLike with Matchers {
  test("test STD LIB section CaseClasses 0. Case classes have an automatic equals method that works:") {
  ...
  test("test STD LIB section CaseClasses 1. Case classes have an automatic hashcode method that works:") {
    case class Person(first: String, last: String)
  ...
  test("test STD LIB section CaseClasses 2. Case classes can be created in a convenient way:") {
  ...
  test("test STD LIB section CaseClasses 3. Case classes have a convenient toString method defined:") {
    case class Dog(name: String, breed: String)
    val d1 = Dog("Scooby", "Doberman")
    d1.toString should be("Dog(Scooby,Doberman)")
  }

  test("test STD LIB section CaseClasses 4. Case classes have automatic properties:") {
    case class Dog(name: String, breed: String)
  ...
  test("test STD LIB section CaseClasses 5. Case classes can have mutable properties:") {
  ...
  test("test STD LIB section CaseClasses 6. There are safer alternatives for altering case classes:") {
  ...
  test("test STD LIB section CaseClasses 7. Case classes can have default and named parameters:") {
  ...
  test("test STD LIB section CaseClasses 8. Case classes can be disassembled to their constituent parts as a tuple:") {
    case class Person(first: String, last: String, age: Int = 0, ssn: String = "")
    val p1 = Person("Fred", "Jones", 23, "111-22-3333")
  ...
  test("test STD LIB section CaseClasses 9. Case classes are Serializable:") {
